Если убрать этап 0, C и Assembler, то план становится ориентированным на создание ОС через Rust (или другой системный язык высокого уровня). Это более современный путь: меньше ручного управления памятью и меньше работы с низкоуровневым кодом, но архитектуру ОС всё равно придётся изучать.

Цель: создать небольшую ОС с ядром, памятью, процессами, файловой системой и драйверами.

  

Этап 1. Архитектура компьютера (2 месяца)
